...
- drain 语义（`crates/session/src/runner/mod.rs::run_loop`）：每个 turn 边界提升 steer；idle 时消费恰好一条 queue。doom-loop 守卫（`DOOM_THRESHOLD=3`，见 `runner/event.rs`）打破连续空 turn 循环。
