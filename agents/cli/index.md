...
## 职责
clap 命令前端 + headless 运行时。解析全局 flag 与子命令（run/tui/ts/server/client/config/models/session/update/install-tools，`ts` 别名 `rs`），把用户意图分发到 session/web/store 层。headless 模式（`run` 或裸 prompt）是 e2e 与脚本化的主入口。
