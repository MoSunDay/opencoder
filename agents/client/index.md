...
- `Remote` 操作面扩展（`src/remote_ops.rs`，18 个方法）：`get_session` / `delete_session` / `fork_session` / `post_compact` / `post_handoff` / `post_skill` / `post_annotation` / `post_autopilot` / `get_models` / `get_skills` / `get_config` / `patch_config` / `list_questions` / `answer_question` / `skip_question` / `list_inputs` / `delete_input` / `reorder_inputs`——与 web 新端点一一对应（见 [agents/web](../web/index.md)）。
