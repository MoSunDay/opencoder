...
- `Config`（`src/config.rs`）：除模型、Provider、压缩与网络配置外，维护 `mcp_servers: HashMap<String, McpServerConfig>` 与 `cli: HashMap<String, CliConfig>` 两类可注入集成。两类条目均以 `enabled` 门控，并共享 `InjectionTarget::{Parent,Subagents,All}`（JSON 字段 `inject_to`，缺省 `parent`）；CLI 条目的 `content` 是自由文本 usage contract。`enabled_mcp_servers_for` / `enabled_cli_for` 按 `AgentMode` 纯函数过滤并按名称稳定排序。`resolve_endpoint() -> Result<Endpoint>` 返回 env 解析后的端点（header value 支持 `{VAR}` 间接引用）；`load(workdir)` 按 global → project → env 深度合并，普通 `save(workdir, patch)` 保持项目优先。
...
- 内置 `fk-cli` skill 通过精确 `fk-session --args` 命令执行聚焦移动 UI TODO，并明确禁止 FK MCP、环境探测和 scheduler finalizer。
