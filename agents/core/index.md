...
- `Config`（`src/config.rs`）：`provider/model/small_model/context_limit/max_tokens/agent/compaction/max_steps`。`load(workdir)` 三层合并（project `opencode.json` / `.opencode/config.json` → global）+ 环境变量覆盖（`OPENCODE_MODEL`/`OPENCODE_SMALL_MODEL`/`OPENCODE_CONTEXT_LIMIT`/`OPENAI_BASE_URL`）。`{VAR}` 形式 api_key 解析环境变量。
- `Agent`/`AgentKind`/`ToolFilter` + 4 内置 agent（act/plan/subagent/command）（`src/agent.rs`）。
