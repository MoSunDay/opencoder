...
- `examples/agent-step-runner.rs` — 容器内 session runner：读 step env（PROMPT/STEP_DIR/SESSION_ID/AGENT/HOW_APPEND）、跑 `opencoder_session::run`、写 events.ndjson/transcript.txt/output.json/session.json（running→done/error），退出码 0/1/2
...
- `examples/wasmtime-cli.rs` — rootfs 自带 WASI 运行器，支持 OCI 缓存配置；模块路径之后的参数全部保留为模块 argv。
