...
- `LlmEvent`（`src/event.rs`）：`TextDelta(String)` / `ReasoningDelta(String)` / `ToolCallStart{id,index,name}` / `ToolCallDelta{index,arguments}` / `Completed{text,tool_calls,usage}` / `Retrying{attempt,max}` / `Error(String)`。`Usage { input_tokens, output_tokens, total_tokens }` 从 `usage` JSON 提取。`ReasoningDelta` 由 `client::extract_reasoning` 触发：按优先级接受 `reasoning_content`/`reasoning`/`thinking`/`reasoning_summary`/`chain_of_thought`/`analysis`/`thoughts` 别名字段（字符串或字符串数组拼接），以及 `delta.content` 结构化数组 `[{type:"text"|"thinking"|"reasoning",text|content}]`（按序发射）；`handle_event` 末帧还回退扫描 `choice.message.*` 补上「一次性整段下发」的推理（高档位 max/xhigh 常见格式）。**双发射守卫**：`emit_delta` 返回本帧是否发射过推理，`handle_event` 用跨帧 `streamed_reasoning` 布尔——只要本 turn 已通过 delta 流式下发过推理，`choice.message` 回退即跳过，避免「delta 流式 + 末帧 message 全量」双通道 provider 重复发射（UI Thinking 块重复、工具轮次下 reasoning_buf 重复并持久化回传）。
