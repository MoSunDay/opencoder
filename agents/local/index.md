...
**crate 更名**：原 `crates/cli`（包名 `opencoder-cli`）→ `crates/local`（包名 `opencoder-local`），行为不变。`opencoder-cli` 名称已释放给新的远程管理 CLI（[agents/ctl](../ctl/index.md)，对接 `opencoder-server` 全量 API）；本地前端仍是根二进制 `opencoder` 的前端，另加无行为影响的 `opencoder --cli` 兼容别名（`crates/local/src/lib.rs` 的 `--cli` global bool）。
