...
Plan 固定使用 plan Agent。Execute 可选择 agent/team/dag/brain；逐次模型请求、响应和完整事件归档适用于 Plan 与 Agent 直驱，包括 brain 解析到 Agent 的路径。Team/DAG 保持其运行时详情与 `output_ref` 引用。
...
- `plan_gen::drive` 每次创建新 plan 会话；`executor/agent_drive.rs` 比较当前 Agent 名称和资源内容摘要，只有身份一致且旧会话可加载时 resume。Agent 或资源版本改变时创建新会话，并携带当前方案与前次结果。
...
- `RunTrace::begin` 在调用模型前保存实际 prompt、Agent 名称、资源名称/版本/文件摘要、模型与推理设置；记录本次会话消息和事件起点。`input_snapshot` 同时写入 run 行与 `input.json`。
