...
- 输入提升：`admit_input`（计算单调 admitted_seq）→ `pending_inputs` 查询 → `promote_inputs`（按 admitted_seq 截止批量标记）/ `claim_next_queue`（原子返回+标记单条 queue，供 drain idle 消费）。`delete_input` / `swap_input_order`（交换两行 admitted_seq，无 UNIQUE 约束可直接交换）供 TUI 队列面板删除/重排未消费的 follow-up。
