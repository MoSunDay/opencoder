...
- `profile.rs`：`profile_team` 成员能力画像。
...
- 磁盘游标（`cursor.rs`）：每步从布局重新推导执行位置——无 plan 的 turn → `Plan`；turn 已记入 `meta.turns` → `Closing`；否则「最小无 summary 的 sub-turn」是工作前沿（其缺失成员 result 重新派发；全部有 summary 时重评估最大 sub-turn，覆盖 summary 落盘与 turns 入账之间崩溃的窗口）。因此 **resume = 再跑一次 `run_topic`**，无任何内存状态需要恢复。
...
- `terminal::finish` 一次逻辑步内先写话题终态元信息、再翻 `team_topic_runs` 该 topic 全部行为 `finished`——两写各自幂等，中间崩溃重试收敛。
...
- `crates/team/tests/runtime_flow.rs`（7）：一次对齐完整跑通并断言全盘布局、歧义点名走第二轮追答、持续不对齐 `max_sub_turns` 收束、成员失败容忍并记录、队长 JSON 纠错带反馈重问、永不收敛 `max_turns` 收束、error 话题 resume 不重放已完成工作。
- `crates/team/tests/termination_flow.rs`（2）：cancel 终态+台账翻转、`profile_team` 扇出并落能力画像。
- store 面：`crates/store/tests/team_runs.rs`（upsert 冻结 created_at / finish 全行翻转 / 节点删除级联）与 `store_migrations.rs` 的 v16→v17 迁移；web 面：`crates/web/tests/api_teams.rs`（9）+ SPA `teamItems.test.js` / `team.dom.test.jsx`。
