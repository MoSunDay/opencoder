...
5. 子 Session 只收到当前 TODO、必要恢复摘要和工具合同，产出结构化 Candidate；解析器接受纯 JSON 或全文中唯一一个完整 JSON fence，并拒绝多个 fence 或未 fenced 的外围说明，工具门禁从当前 attempt 的持久化消息独立计算。schema v2 的非空 required tool calls 一旦按序全部成功，运行时立即停止 focused session 并可从硬门禁证据确定性生成 Candidate，避免格式错误尾帧导致重复副作用。
