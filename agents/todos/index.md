...
5. 子 Session 只收到当前 TODO、必要恢复摘要和工具合同，产出结构化 Candidate；解析器接受纯 JSON，或全文中以最后一个裸 ``` 行闭合、其后仅空白的 JSON fence（响应内嵌反引号 fence 也合法；"multiple JSON fences" 仅在解析失败且内容内嵌 ``` 时报），工具门禁从事件记录独立计算。
