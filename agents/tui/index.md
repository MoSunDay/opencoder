...
- **弹窗锚点**：`/` 命令面板（`command.rs`）与 `/config`/`/model` 配置（`model_menu/`）以 composer 顶边 `y` 为底锚渲染下拉浮层（非屏幕居中）。**`/config`**（`ConfigForm`）= 生成参数表单（reasoning 四档循环 / interleave 开关 / max_tokens / context_threshold / fps / capabilities 开关），Enter 推进字段到 `[Save]` 提交；**`/model`**（`ProviderList` + `ProviderForm`）= Provider CRUD：列表 Enter=切换 / `e`=编辑 / `n`=新建 / `d`=删除（y/n 两步确认），编辑表单含 name/model_id/base_url/api_key（掩码+重输）/自定义 headers 子编辑器（`HeadersEditor`：`+`/`-` 增删 pair，`←/→` name↔value）。两者产出 JSON merge-patch 经 `Config::save` 写回 opencoder.json。`/task` 会话选择器（`task.rs`）为屏幕居中模态：`+ New task` + 历史 session 列表（当前会话标 `(current)`），底部红色 `✕ Clear all N task(s)` destructive 行——选中 Enter 进入红色两步确认态（再 Enter 触发 `TaskOutcome::ClearAll`；app 层经 `gate_clear_all(running)` 守卫：`running==true` 即 turn/subagent 飞行中时拒绝并推黄色 busy marker，idle 后才调 `Store::clear_other_sessions` 删除除当前会话外的全部会话并刷新列表；Esc 取消确认、确认态锁定 ↑/↓、Ctrl+C/D 仍即时退出）。
