...
- **Session 事件**：`chat.apply(&sev)` 内部调 `track_context` 更新 `chat.context_used`（只计本 view token，SubagentChild 不计入父），SubagentChild 路由到子 block view（子 view 的 apply 独立更新自身 context_used）；TranscriptReset 重建 ChatView（context_used 归零）。 worker→UI 转发经 `forward_event(&tx, sev)`（`src/worker.rs`）而非裸 `try_send`：有界 mpsc 接近满（`tx.capacity() <= DELTA_MIN_CAPACITY=64`）时丢弃可恢复的流式 delta（`TextDelta`/`ReasoningDelta`/包裹它们的 `SubagentChild`）；最终文本总在 `TurnDone → finalize_assistant()` 从 store 重建，故无数据丢失；生命周期事件（TurnDone/Error 等）始终 `try_send` 优先占槽。
