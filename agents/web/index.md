...
- `spa/src/brain/workbench/` — schema 6 工作台。`scheduler/editor.jsx` 在 `milestone/` 画布上配置层级、里程碑与能力，随后用表单提交计划信息；`milestone/run.jsx` 按轮次、层和激活展示调度及执行 ID，并复用 `ExecutionView` 查看各类型明细。`useRun.js` 读取 `/layered`，由事件流及轮询刷新。
- `tests/` — 集成测试
