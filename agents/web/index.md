...
- `spa/src/brain/workbench/` — 仅 v4。`scheduler/editor.jsx` 编辑节点/能力和连线；`layered/` 自上而下展示层级，节点点击复用 ExecutionView；`useRun.js` 直接读取 `/layered`，事件流及 3 秒轮询刷新，`rounds.jsx` 按需取层索引。画布节点固定 216×156，布局常量与 CSS 同步，为重试、取消标记和上游信息预留空间，标题不参与纵向压缩。
- `tests/` — 集成测试
