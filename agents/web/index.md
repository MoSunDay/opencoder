...
- 会话列表（`src/api.rs`）：`GET /api/sessions` 增 `?workdir=` 过滤（经 `opencoder_core::data_dir::workdir_hash`，新会话行打戳；旧 NULL-hash 会话不被匹配）；`POST /agent` 切到 plan 时发 `ResetPlanPhase` 并持久化 `plan_input_count=0`。事件流端点（`get_events`/`get_event_seq`）已拆至 `src/api_events.rs`（api.rs 行数预算；经 `api.rs` re-export 保持路径稳定）；`GET /events` 增 `Last-Event-ID` header 回退（无 `?after=` 时读 header）。
