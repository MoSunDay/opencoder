...
- SPA 前端 `GET /`（`src/html.rs`）：React18 + antd 6 + @ant-design/x + Vite 源码在 `spa/src`（单测 vitest），构建产物**提交**在 `spa/dist`（`index.html` + `static/app.js`/`static/app.css` 固定文件名、无内容哈希），经 `include_str!`/`include_bytes!` 编译期内嵌——`cargo build` 永不需要 node；路由 `/` + `/static/:name` 白名单伺服（白名单外 404）；构建 `scripts/build-spa.sh`、漂移检查 `scripts/check-spa-drift.sh`。会话面板为 X chat 三件套：`Bubble.List`（`transcript.jsx`+纯映射 `bubbleItems.js`）+ `Sender`（Enter 提交、loading 停止键=interrupt）+ `Conversations` 侧栏（`chatSidebar.jsx`+纯映射 `conversationItems.js`，本机/远端双源对话）；协议层 `sign/sse/reduce` 冻结不涉 UI。所有 `/api` 调用经 `spa/src/sign.js` 按共享 token 携带 `x-sig-timestamp`/`x-sig` 签名；`crypto.subtle` 只在 secure context 存在（LAN IP 明文 `http://` 下为 undefined，签名必炸）——`sign.js` 探测缺失即回退 `spa/src/sha256.js` 纯 JS SHA-256/HMAC（向量对齐 NIST/RFC4231 与 Rust 侧常量），内网 HTTP 原生可用、明文警示 banner 已移除；`GET /api/time`（`server_time_ms`）供浏览器校准时钟偏移。SSE 重连契约（`spa/src/sse.js`）：按 `?after=lastSeq` 做**有界尾部重放**，`REPLAY_CAP_FRAMES=400` 封顶且终帧恒为 head（无封顶时 4 万帧回放 O(n²) 会冻结页面；整体 resync 机制已删除）；`GET /api/sessions/:id` 响应含 `draining: bool`（compact 等长收尾可观测）；wire `ContentBlock` 是 serde `tag = "kind"`（`crates/core/src/message.rs`），SPA 快照渲染兼容 `kind`/`type` 双 tag。
...
- feature-parity 端点测试：`tests/web_api_ops.rs`（fork/skill/compact/handoff/config/bg）；SPA 装配单测：`src/html.rs`。 环境管理端点：`tests/web_envs.rs`（列表/创建/激活/重捕获/删除 + ReloadConfig 扇出）。子代理列表/clear-all（2026-08-25）：`tests/subagent_list_api.rs`、`tests/clear_sessions_api.rs`；SPA 单测在 `spa/src`（vitest 7 文件 56 用例：sign/reduce/bubbleItems/conversationItems 纯函数 + app/chat/sidebar jsdom 冒烟）。
