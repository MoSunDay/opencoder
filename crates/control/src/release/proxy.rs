...
pub async fn status(State(state): State<Arc<AppState>>) -> Response {
    let result = async {
        ...
        Ok::<_,anyhow::Error>(serde_json::json!({"instance_release":platform.release_id,"release":release,"host":host,"retiring":state.lifecycle.retiring.load(std::sync::atomic::Ordering::SeqCst)}))
    }.await;
    match result {
