...
pub fn build_app(state: Arc<AppState>, token: Option<String>, web: bool) -> Router {
    ...
    let mut app = Router::<Arc<AppState>>::new()
        ...
        .route("/api/envs", get(api_envs::list).post(api_envs::create).patch(api_envs::activate))
        .route("/api/envs/:name", get(api_envs::detail).patch(api_envs::update).put(api_envs::update).delete(api_envs::delete))
