...
use opencoder_core::{fleet::*, identity::{Identity, Role}};
use serde_json::{json, Value};
use tower::ServiceExt;

fn registration(id: &str) -> NodeRegistration {
    NodeRegistration {
        id: id.into(), name: id.into(), version: "test".into(),
        protocol_version: PROTOCOL_VERSION, maintenance_agent_id: "act".into(),
...
async fn delete_registration_keeps_indexes_and_survives_reopen() {
    ...
    let state = opencoder_control::new_state(work.clone(), data.clone(), None).await.unwrap();
    for id in ["node-a", "node-b"] { state.fleet.register(&registration(id)).await.unwrap(); }
    let record = ExecutionIndex { id: "agent-history".into(), created_at: 7,
        kind: ExecutionKind::Agent, node_id: "node-a".into(), status: ExecutionStatus::Running };
    ...
    assert_eq!(call(&app, "GET", "/api/nodes").await.1["nodes"].as_array().unwrap().len(), 2);
    assert_eq!(call(&app, "DELETE", "/api/nodes/node-a").await, (200, json!({"ok":true})));
    ...
    let state = opencoder_control::new_state(work, data, None).await.unwrap();
    assert_eq!(state.hub.views().await.len(), 1);
    assert_eq!(serde_json::to_value(state.fleet.index(&record.id).await.unwrap().unwrap()).unwrap(), json!(record));
    ...
    assert_eq!(state.fleet.indexes(Some("node-a"), None, 10).await.unwrap().len(), 1);
}

#[tokio::test]
async fn node_registration_delete_requires_admin_identity() {
    let dir = tempfile::tempdir().unwrap();
    let mut state = opencoder_control::new_state(dir.path().join("work"), dir.path().join("data"), None).await.unwrap();
    state.fleet.register(&registration("node-a")).await.unwrap();
    Arc::get_mut(&mut state).unwrap().hub = Arc::new(opencoder_control::transport::Hub::new(vec![registration("node-a")]));
    let app = opencoder_control::build_app(state.clone(), Some("registration-test".into()), false);
    let response = app.oneshot(Request::builder().method("DELETE").uri("/api/nodes/node-a")
        .body(Body::empty()).unwrap()).await.unwrap();
    ...
    for role in [Role::User, Role::Root] {
        ...
        let mut request = Request::builder().method("DELETE").uri("/api/nodes/node-a").body(Body::empty()).unwrap();
        request.extensions_mut().insert(Identity { name: "user".into(), role });
