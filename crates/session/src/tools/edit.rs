...
impl Tool for EditTool {
    ...
    fn parameters(&self) -> Value {
        ...
        props.insert("old_string".into(), json::prop_str("The exact text to replace."));
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if count > 1 && !replace_all {
            return Ok(ToolOutput::err(format!("old_string matches {count} times; set replace_all=true or add context")));
        }
        ...
        Ok(ToolOutput::ok(format!("edited {} ({} replacement{})", full.display(), if replace_all { count } else { 1 }, if count == 1 { "" } else { "s" })))
    }
}
