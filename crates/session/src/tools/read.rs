...
impl Tool for ReadTool {
    ...
    fn parameters(&self) -> Value {
        ...
        props.insert("path".into(), json::prop_str("Path to the file to read, relative to the working directory."));
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let offset = input.get("offset").and_then(|v| v.as_u64()).unwrap_or(0).max(1) as usize;
...
pub(crate) fn resolve(ctx: &ToolContext, path: &str) -> std::path::PathBuf {
    ...
    if p.is_absolute() { p.to_path_buf() } else { ctx.working_dir.join(p) }
}
