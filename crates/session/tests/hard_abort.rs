...
async fn cancel_hard_aborts_a_running_tool() {
    ...
    let waiter = tokio::spawn(async move {
        // Let the LLM call resolve and the bash tool start, then cancel
        // mid-tool.
    ...
    // Store-less hard-abort path: the whole batch is recorded (there is no
    // store to replay anything from, so no id may stay dangling) — otherwise
    // the next LLM request in this same process would hit the provider's
    // "unanswered tool_call" HTTP 400.
    ...
    assert!(
        dangling.is_empty(),
        "hard-aborted tool_use ids must be answered even without a store: {dangling:?}"
    );
    let bash_recorded = s.messages.iter().any(|m| {
        m.blocks.iter().any(|b| {
            matches!(
                b,
    ...
    assert!(
        bash_recorded,
        "the interrupted bash result must be recorded (Fix A store-less path)"
    );
}
