...
fn config(model: &str) -> Config {
    Config { model: model.into(), ..Config::default() }
}
...
fn done_event(text: &str) -> LlmEvent {
    LlmEvent::Completed {
        ...
        usage: Some(Usage { input_tokens: 5, output_tokens: 3, total_tokens: 8 }),
    }
}
...
async fn resume_reconstructs_history_byte_equal() {
    ...
    let original = run_with_store(store.clone(), "sess-A", "main/glm-5.2", "hello world", mock.clone() as Arc<dyn ChatStream>).await;
    ...
    assert_eq!(resumed.messages.len(), original.messages.len(), "history length must match");
...
async fn continue_picks_latest_session() {
    let store: Arc<dyn Store> = Arc::new(mem_store().await);
    run_with_store(store.clone(), "older", "m", "old prompt", client_done("o") as Arc<dyn ChatStream>).await;
    // tiny delay so created_at differs
    tokio::time::sleep(std::time::Duration::from_millis(20)).await;
    run_with_store(store.clone(), "newer", "m", "new prompt", client_done("n") as Arc<dyn ChatStream>).await;
...
async fn cross_process_resume_reads_full_history() {
    ...
    // process 1: open file store, run a session
    {
        ...
        run_with_store(store, "cp-sess", "m", "persisted across processes", client_done("ok") as Arc<dyn ChatStream>).await;
    }
    // process 2: reopen the SAME file, resume
    let store2: Arc<dyn Store> = Arc::new(LibsqlStore::open(&db_path).await.unwrap());
    let resumed = resume(store2.clone(), "cp-sess", config("m"), client_done("y") as Arc<dyn ChatStream>, std::path::PathBuf::from("/tmp")).await.unwrap();
    let texts: Vec<String> = resumed.messages.iter().map(|m| m.text()).collect();
    assert!(texts.iter().any(|t| t.contains("persisted across processes")), "must find the original user msg");
    assert!(texts.iter().any(|t| t == "ok"), "must find the original assistant reply");
}
...
async fn persistence_on_runner_path_writes_all_roles() {
    let store: Arc<dyn Store> = Arc::new(mem_store().await);
    let mock = Arc::new(
        MockChatClient::new().push_script(vec![LlmEvent::Completed {
    ...
    let dir = tempfile::tempdir().unwrap();
    let agent = resolve_agent("act").unwrap();
    let mut s = SessionState::new("persist-all", agent, config("m"), mock, dir.path().to_path_buf())
    ...
    let has_tool_use = on_disk.iter().any(|m| {
        m.blocks.iter().any(|b| matches!(b, ContentBlock::ToolUse { .. }))
    });
...
async fn title_generation_writes_via_small_model() {
    ...
    let mut s = run_with_store(store.clone(), "t", "main/glm", "build me a snake game", mock.clone() as Arc<dyn ChatStream>).await;

    // small-model title call: capture request, return a short title
    let title_mock = Arc::new(MockChatClient::new().push_script(vec![done_event("Snake Game Builder")]));
    ...
    assert_eq!(meta.title.as_deref(), Some("Snake Game Builder"), "title persisted to store");
}

#[tokio::test]
async fn fork_does_not_mutate_parent() {
    let store: Arc<dyn Store> = Arc::new(mem_store().await);
    let parent = run_with_store(store.clone(), "parent", "m", "original task", client_done("p0") as Arc<dyn ChatStream>).await;

    // fork: copy parent's messages into a NEW session id, then run more on the child
    let child_msgs = parent.messages.clone();
    store.create_session(&opencode_store::SessionMeta {
        id: "child".into(),
    ...
    let mut child = resume(store.clone(), "child", config("m"), client_done("child-extra"), parent.working_dir.clone()).await.unwrap();
    run(&mut child, "extend the task".into(), |_| {}).await.unwrap();
    ...
    assert!(child_disk.len() > parent_disk.len(), "child grew beyond parent");
    assert_eq!(parent_disk.last().map(|m| m.text()), Some("p0".to_string()), "parent's last msg unchanged");
}
