...
impl Store for LibsqlStore {
    ...
    async fn get_brain_playbook(&self, id: &str) -> Result<Option<BrainPlaybookRecord>> {
        let _guard = self.db_lock.lock().await;
        brain_playbooks::get(&self.conn, id).await
    }

    async fn list_brain_playbooks(&self) -> Result<Vec<BrainPlaybookRecord>> {
        let _guard = self.db_lock.lock().await;
        brain_playbooks::list(&self.conn).await
    }

    async fn delete_brain_playbook(&self, id: &str) -> Result<bool> {
        let _guard = self.db_lock.lock().await;
        brain_playbooks::delete(&self.conn, id).await
    }

    async fn latest_brain_playbook_for(&self, digest: &str) -> Result<Option<BrainPlaybookRecord>> {
        let _guard = self.db_lock.lock().await;
        brain_playbooks::latest_by_digest(&self.conn, digest).await
    }
