...
async fn schema_migration_versioning() {
    ...
    assert_eq!(v, 21, "schema_version must be latest (21) after bootstrap");
}
...
async fn schema_migration_v1_to_v2_adds_sse_kind() {
    ...
    // Schema version bumped to 2.
    {
        ...
        let v: i64 = r.get(0).unwrap();
        assert_eq!(v, 21, "schema version must be latest (21) after migration");
    }
    ...
    let r = rows.next().await.unwrap().unwrap();
    let v: i64 = r.get(0).unwrap();
    assert_eq!(v, 21, "schema version stays 21 after idempotent re-open");
}
...
async fn schema_migration_v2_to_v3_adds_handoff_and_skill() {
    ...
    // (3) Schema version bumped to 3.
    {
        ...
        assert_eq!(
            v, 21,
            "schema version must be latest (21) after v2→v3 migration"
        );
    }
