...
fn validates_every_required_file_and_returns_file_locations() {
    ...
    assert!(errors.iter().any(|e| e.path == "workflow.json" && e.line == 2));
...
fn atomic_versions_and_legacy_import_keep_the_source_unchanged() {
    ...
    files.insert("todos/first/instructions.md".into(), "new instructions".into());
    let next = root.path().join("v2");
    directory::write_new(&next, &files).unwrap();
    assert_eq!(directory::load(&next).unwrap().todos[0].instructions, "new instructions");
    assert_eq!(std::fs::read_to_string(old.join("context.json")).unwrap(), original);
...
fn unknown_fields_and_invalid_dependencies_fail_without_dropping_data() {
    let mut files = directory::encode(&spec(), None).unwrap();
    let mut task: serde_json::Value = serde_json::from_str(&files["todos/first/task.json"]).unwrap();
    task["typo"] = json!(true);
    files.insert("todos/first/task.json".into(), task.to_string());
    assert!(directory::validate(&files).iter().any(|e| e.path == "todos/first/task.json" && e.message.contains("typo")));
    task.as_object_mut().unwrap().remove("typo");
    task["depends_on"] = json!(["missing"]);
    files.insert("todos/first/task.json".into(), task.to_string());
    assert!(directory::validate(&files).iter().any(|e| e.path == "todos/first/task.json" && e.message.contains("missing")));
}
...
fn rejects_symlink_files_and_directories() {
    ...
    assert!(directory::read_files(root.path()).unwrap_err().to_string().contains("symlinks"));
}
