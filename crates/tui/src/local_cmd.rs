...
//! render their result directly into the transcript as a purple marker. They
//! never call `record()`, never start a turn, and never reach
...
pub(crate) fn run(text: &str, chat: &mut ChatView) -> bool {
    if !is_local(text) {
        return false;
    }
