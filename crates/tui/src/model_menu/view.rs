...
fn render_config_form(f: &mut Frame, area: Rect, composer_top: u16, form: &ConfigForm) {
    ...
    let threshold_hint = format!("{} tokens (\u{2248}{}k)", form.threshold, form.threshold / 1000);
    let reasoning_val = format!("[ {} ]", form.reasoning.label());
    let interleave_val = format!("[ {} ]", if form.interleaved_thinking { "on" } else { "off" });
    ...
    let lines = vec![
        field_line("thinking:", &reasoning_val, form.focus == ConfigField::Reasoning, "\u{2190}/\u{2192}/Space cycle, Enter=next"),
        field_line("interleave:", &interleave_val, form.focus == ConfigField::InterleavedThinking, "\u{2190}/\u{2192}/Space toggle, Enter=next"),
        field_line("max_tokens:", &max_tokens_val, form.focus == ConfigField::MaxTokens, "digits, Backspace, empty=unset, Enter=next"),
        field_line("ctx threshold:", &threshold_hint, form.focus == ConfigField::Threshold, "digits/\u{2190}\u{2192} \u{00b1}1k, Enter=next"),
        field_line("fps:", &format!("{} FPS", form.fps), form.focus == ConfigField::Fps, "1-30, digits/\u{2190}\u{2192} \u{00b1}1"),
        field_line("browser:", &format!("[ {} ]", if form.capabilities_browser { "on" } else { "off" }), form.focus == ConfigField::Browser, "\u{2190}/\u{2192}/Space toggle"),
        field_line("computer_use:", &format!("[ {} ]", if form.capabilities_computer_use { "on" } else { "off" }), form.focus == ConfigField::ComputerUse, "\u{2190}/\u{2192}/Space toggle"),
        field_line("tools_sub:", &format!("[ {} ]", if form.capabilities_tools_subagent { "on" } else { "off" }), form.focus == ConfigField::ToolsSubagent, "\u{2190}/\u{2192}/Space toggle"),
...
fn render_provider_list(f: &mut Frame, area: Rect, composer_top: u16, list: &ProviderList) {
    ...
    if list.entries.is_empty() {
        lines.push(Line::styled(" No providers configured.", Style::default().fg(Color::Yellow)));
        lines.push(Line::styled(" Press 'n' to add one, or edit opencoder.json.", dim_style()));
    } else {
...
fn render_provider_form(f: &mut Frame, area: Rect, composer_top: u16, form: &ProviderForm) {
    ...
    let title = match &form.error {
        ...
        None => format!(" /model {mode} \u{2014} type values, Enter=next, [Save] commits, Esc cancel "),
    };
    let block = Block::default().borders(Borders::ALL).title(title);
    ...
    let name_display = if form.name_readonly {
        ...
        if form.name.is_empty() { "(empty)".to_string() } else { form.name.clone() }
    };
    let model_display = if form.model_id.is_empty() { "(empty)".to_string() } else { form.model_id.clone() };
    let base_display = if form.base_url.is_empty() { "(empty)".to_string() } else { form.base_url.clone() };

    let mut lines = vec![
        field_line("name:", &name_display, form.focus == ProviderField::Name && !form.name_readonly, "type provider name, Enter=next"),
        field_line("model_id:", &model_display, form.focus == ProviderField::ModelId, "type model id, Enter=next"),
        field_line("base_url:", &base_display, form.focus == ProviderField::BaseUrl, "type URL, Enter=next"),
        field_line("api_key:", &form.api_key_display(), form.focus == ProviderField::ApiKey, "type new value, Enter=next"),
    ];
    ...
    lines.push(field_line("headers:", &format!("({} pair{})", form.headers.pairs.len(), if form.headers.pairs.len() == 1 { "" } else { "s" }), form.focus == ProviderField::Headers, &hdr_hint));

    if form.headers_active || form.focus == ProviderField::Headers {
        if form.headers.pairs.is_empty() {
            lines.push(Line::styled("     (no headers, press + to add)", Style::default().fg(Color::DarkGray)));
        }
        for (i, (hn, hv)) in form.headers.pairs.iter().enumerate() {
            ...
            let val_disp = if hv.is_empty() { "(value)" } else { hv.as_str() };
            let style = if selected && form.headers_active { focus_style() } else { dim_style() };
