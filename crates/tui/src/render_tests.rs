    ...
    fn status_bar_omits_branding() {
        ...
        assert!(row.contains("[act]"), "agent chip should appear; got: {row}");
        assert!(row.contains("myproj"), "dir name should appear; got: {row}");
        assert!(row.contains("ctx"), "context indicator should appear; got: {row}");
    }
    ...
    fn status_bar_running_shows_spinner_and_status() {
        ...
        let row = row_text(terminal.backend().buffer(), 0, 120);
        assert!(row.contains("thinking"), "status text should appear; got: {row}");
        assert!(
            row.contains('\u{280b}'),
    ...
    fn header_line_indices_aligned_with_flatten_while_withheld() {
        ...
        v.apply(&SessionEvent::TextDelta("preamble line one\npreamble line two".into()));
        ...
        v.apply(&SessionEvent::ReasoningDelta("post\ndispatch\nanalysis".into()));

        assert!(v.hidden_assistant_idx.is_some(), "preamble must be withheld");
        assert_eq!(v.subagents_running, 2);
        let flat = v.flatten_with(0);

        let line_text = |idx: usize| -> String {
            flat[idx].spans.iter().map(|s| s.content.clone()).collect()
        };
