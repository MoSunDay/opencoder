...
/// The turn cost timer appears at the tail — after the content text on the same line.
#[test]
fn body_turn_cost_timer_after_content() {
    let full = render_body_with_tail("hello world", 42000, 60, 8);
    for row in full.lines() {
        if let Some(content_pos) = row.find("hello world") {
            let timer_pos = row.find("[turn cost 42s]");
            assert!(
...
#[test]
fn body_turn_cost_timer_wraps_to_own_line_when_full() {
