    ...
    fn multiple_subagents_withhold_output_until_all_done() {
        ...
        assert!(v.hidden_assistant_idx.is_some(), "preamble hidden once 2 run");
        assert!(
        ...
        assert!(v.hidden_assistant_idx.is_none(), "preamble revealed once all done");
        let text = block_text(&v);
        assert!(text.contains("launching investigators"), "preamble reappears");
        assert!(text.contains("result-a"), "first summary shown after flush");
        assert!(text.contains("result-b"), "second summary shown after flush");
    }
    ...
    fn single_subagent_does_not_withhold() {
        ...
        assert!(block_text(&v).contains("preamble"), "preamble still visible");
    ...
    fn done_while_subagents_running_reveals_preamble() {
        ...
        assert!(v.hidden_assistant_idx.is_none(), "Done must reveal preamble");
        assert!(v.pending_subagent_ends.is_empty(), "Done must flush pending");
