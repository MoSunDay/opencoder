...
const StepNode = memo(function StepNode({ data, selected }) {
  ...
  return <div className={`brain-node brain-node-${status} ${selected ? 'brain-node-selected' : ''}`}>
    ...
    {data.ontology ? <div className="brain-node-contract">{data.step ? <><span>输入 {Object.keys(data.step.inputs || {}).join(' · ') || '无'}</span><span>输出 {data.step.output?.semantic || data.step.output?.type}</span></> : data.description || data.port?.schema?.semantic || data.port?.schema?.type}</div>
...
export function PlanCanvas({ plan, groups, capabilities, mode = 'execution', selected, onSelect, positions = {}, viewport, onPositions, onViewport, onConnect }) {
  ...
  return <div className="brain-canvas" aria-label={mode === 'ontology' ? '本体关系画布' : '步骤执行画布'}>
    <ReactFlow nodes={nodes} edges={projection.edges} nodeTypes={nodeTypes} nodesDraggable={!!onPositions} nodesConnectable={!!onConnect} onConnect={onConnect} isValidConnection={({ source, target }) => plan.steps.some((s) => s.id === source) && plan.steps.some((s) => s.id === target)} onNodesChange={(changes) => { const moved = changes.filter((c) => c.type === 'position' && c.position); if (moved.length) onPositions?.(Object.fromEntries(moved.map((c) => [c.id, c.position]))); }} onMoveEnd={(_, value) => onViewport?.(value)} defaultViewport={viewport || undefined} onNodeClick={(_, node) => onSelect?.(node.id)} fitView={!viewport} minZoom={0.2} maxZoom={2} onlyRenderVisibleElements>
