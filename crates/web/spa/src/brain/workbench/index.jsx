...
export function BrainWorkbench({ onNotice }) {
  ...
  return <PageShell page="brain">{runId ? <BrainRunView key={runId} id={runId} onBack={back} onNotice={onNotice} /> : <>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: 'workspace', label: '工作台', children: <><Space style={{ marginBottom: 16 }}><Button type="primary" onClick={() => { setInitialPlan(null); setLaunch(true); }}>开始新任务</Button><Button onClick={reload}>刷新运行</Button><Button disabled={!runCursor} onClick={olderRuns}>更早运行</Button></Space><Table rowKey="id" dataSource={runs} pagination={{ pageSize: 10 }} columns={[
