...
export function BrainWorkbench({ onNotice }) {
  ...
  return <PageShell page="brain">{runId ? <BrainRunView key={runId} id={runId} onBack={back} onNotice={onNotice} /> : <>
    ...
    <Tabs activeKey={tab} onChange={setTab} items={[
      { key: 'workspace', label: '工作台', children: <><Space style={{ marginBottom: 16 }}><Button type="primary" onClick={() => setLaunch(true)}>开始新任务</Button><Button onClick={reload}>刷新运行</Button><Button disabled={!runCursor} onClick={olderRuns}>更早运行</Button></Space><Table rowKey="id" dataSource={runs} pagination={{ pageSize: 10 }} columns={[
      ...
      { key: 'plans', label: '计划库', children: <Plans plans={plans} capabilities={capabilities} reload={reload} readOnly /> },
      { key: 'capabilities', label: '能力库', children: <BrainPanel onNotice={onNotice} /> },
    ]} />
  </>}
  <Drawer destroyOnHidden open={launch} onClose={() => setLaunch(false)} title="新建大脑运行" size={600}><Launch capabilities={capabilities} onCreated={(id) => { setLaunch(false); setRunId(id); }} /></Drawer>
  </PageShell>;
}
