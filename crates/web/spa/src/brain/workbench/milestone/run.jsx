...
export function MilestoneRunBody({ view, id, refresh, onNotice }) {
  ...
  const { run, plan } = view; const history = visits(view); const visit = history.find((v) => v.activation === selectedVisit) || history.at(-1);
  ...
  const statuses = Object.fromEntries(plan.nodes.map((node) => {
    ...
    const label = assessment ? (assessment.met ? '里程碑已达标' : '里程碑需整改') : !ops.length ? '本次未派发' : ops.some((op) => !['done', 'error', 'cancelled'].includes(op.status)) ? '执行中' : ops.some((op) => op.status !== 'done') ? '执行有失败，等待判断' : '执行结束';
  ...
  return <>
    ...
    <div className="brain-milestone-preview"><MilestoneCanvas plan={plan} statuses={statuses} onSelect={(nodeId) => { const op = operations.find((item) => item.node_id === nodeId); setExecutionId(op?.execution_id || null); }} /></div>
    <h3>轮次、层级与能力执行</h3>
