...
export function graph(plan, groups = [], ontology = false) {
  const nodes = (plan?.steps || []).map((step) => ({ id: step.id, type: 'brainStep', data: { step, label: step.label, kind: step.action.kind, group: groups.find((g) => g.id === step.id), ontology } }));
  const edges = (plan?.steps || []).flatMap((s) => dependencies(s).map((dep) => ({ id: `${dep}->${s.id}`, source: dep, target: s.id, type: 'smoothstep', style: s.when ? { strokeDasharray: '5 4' } : {}, label: s.when ? '条件' : s.foreach ? '展开' : '' })));
  if (ontology) {
  ...
  nodes.forEach((n) => layout.setNode(n.id, { width: 224, height: ontology ? 128 : 104 })); edges.forEach((e) => layout.setEdge(e.source, e.target)); dagre.layout(layout);
  return { nodes: nodes.map((n) => ({ ...n, position: { x: layout.node(n.id).x - 112, y: layout.node(n.id).y - 52 } })), edges };
}
