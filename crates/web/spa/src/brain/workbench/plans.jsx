...
export function Plans({ plans, capabilities, reload, onRun, readOnly = false }) {
  ...
  const open = async (p, edit = false) => { try { const version = await apiGet(`/api/brain/plan-defs/${encodeURIComponent(p.id)}/versions/${p.latest_version}`); if (edit && version.plan.schema_version !== 2) throw new Error("旧计划只读，请显式新建 v2 计划"); if (edit) setEditor({ ...version, version: p.latest_version + 1 }); else { setView(version); setDiff(null); const history = await apiGet(`/api/brain/plan-defs/${encodeURIComponent(p.id)}/versions`); setVersions(history.versions); } } catch (e) { setError(e.message); } };
  ...
  return <>{readOnly && <Alert type="info" showIcon title="历史计划只读；新任务请从工作台开始调度" />}<Space style={{ marginBottom: 16 }}><Input.Search placeholder="搜索计划名称" value={search} onChange={(e) => setSearch(e.target.value)} />{!readOnly && <Button type="primary" onClick={() => setEditor({ creating: true })}>新建计划</Button>}</Space>
