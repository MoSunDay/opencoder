...
export function Plans({ plans, capabilities, reload, onRun }) {
  ...
  const open = async (p, edit = false) => {
    try {
      ...
      if (edit) { if (version.plan.schema_version !== 3) throw new Error('历史计划只读，请新建调度计划'); setEditor(version); }
  ...
  return <><Space style={{ marginBottom: 16 }}><Input.Search placeholder="搜索计划名称" value={search} onChange={(event) => setSearch(event.target.value)} /><Button type="primary" onClick={() => setEditor({ creating: true })}>新建计划</Button></Space>
    ...
    <Table rowKey="id" pagination={{ pageSize: 10 }} dataSource={plans.filter((p) => `${p.title} ${p.id}`.toLowerCase().includes(search.toLowerCase()))} columns={[
      ...
      { title: '版本', dataIndex: 'latest_version', render: (v, p) => <Space>v{v}{p.schema_version !== 3 && <Tag>历史只读</Tag>}</Space> },
      { title: '操作', render: (_, p) => p.schema_version === 3 && <Space><Button size="small" onClick={() => open(p, true)}>创建下一版本</Button><Button size="small" onClick={() => onRun(`${p.id}@${p.latest_version}`)}>执行</Button></Space> },
    ...
    <Drawer open={!!view} onClose={() => setView(null)} title={view?.plan.title} size="85vw">{view && <><Space wrap>
      ...
      {view.plan.schema_version === 3 ? <Button onClick={() => { onRun(`${view.id}@${view.version}`); setView(null); }}>执行此版本</Button> : <Tag>历史计划只读</Tag>}
      ...
      {view.plan.schema_version === 3 ? <><SummaryCanvas view={{ ...view.plan, capabilities: capabilities.filter((cap) => view.plan.capability_ids.includes(cap.id)) }} /><Typography.Paragraph>能力 ID：{view.plan.capability_ids.join('、')}</Typography.Paragraph><Typography.Text>最多 {view.plan.max_rounds} 轮</Typography.Text></> : <PlanCanvas plan={view.plan} mode="ontology" />}
    ...
    <Drawer className="brain-plan-drawer" title={editor?.creating ? '新建计划' : '编辑计划新版本'} placement="right" size="100%" open={!!editor} onClose={() => editorRef.current?.close()} destroyOnHidden styles={{ body: { padding: 0 } }}>
      {editor && <PlanEditor ref={editorRef} key={draftKey(owner, editor.creating ? null : editor)} cacheKey={draftKey(owner, editor.creating ? null : editor)} version={editor.creating ? undefined : editor} capabilities={capabilities} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); await reload(); }} />}
    </Drawer>
  </>;
}
