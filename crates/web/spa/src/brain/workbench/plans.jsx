...
export function Plans({ plans, capabilities, reload, onRun }) {
  ...
  const open = async (p, edit = false) => {
    try {
      ...
      if (edit) { if (![4, 5, 6].includes(version.plan.schema_version)) throw new Error('不支持的计划版本'); setEditor(version); }
  ...
  return <><Space style={{ marginBottom: 16 }}><Input.Search placeholder="搜索计划名称" value={search} onChange={(event) => setSearch(event.target.value)} /><Button type="primary" onClick={() => setEditor({ creating: true })}>新建计划</Button>
    ...
    <Table rowKey="id" pagination={{ pageSize: 10 }} dataSource={plans.filter((p) => `${p.title} ${p.id}`.toLowerCase().includes(search.toLowerCase()))} columns={[
      ...
      { title: '版本', dataIndex: 'latest_version', render: (v, p) => <Space>v{v}{p.schema_version !== 6 && <Tag>{p.schema_version === 4 ? "历史 DAG" : "不支持"}</Tag>}</Space> },
      { title: '操作', render: (_, p) => [4, 5, 6].includes(p.schema_version) && <Space><Button size="small" onClick={() => open(p, true)}>{p.schema_version === 4 ? "转换为里程碑版本" : "创建下一版本"}</Button><Button disabled={p.schema_version !== 6} size="small" onClick={() => onRun(`${p.id}@${p.latest_version}`)}>执行</Button></Space> },
    ...
    <Drawer open={!!view} onClose={() => setView(null)} title={view?.plan.title} size="85vw">{view && <><Space wrap>
      ...
      {view.plan.schema_version === 6 ? <Button onClick={() => { onRun(`${view.id}@${view.version}`); setView(null); }}>执行此版本</Button> : <Tag>不支持的计划版本</Tag>}
