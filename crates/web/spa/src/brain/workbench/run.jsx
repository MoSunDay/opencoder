...
export function BrainRunBody({ id, onNotice, header = null }) {
  ...
  const select = (step) => { setStepId(step); setInstanceId(run?.groups.find((g) => g.id === step)?.current_instance || run?.instances.find((i) => i.step_id === step)?.id || null); };
  ...
  return <div className="brain-run">
    ...
    {!run ? <Spin /> : <><div className="brain-run-header"><div><Typography.Title level={4}>{run.objective}</Typography.Title><Space wrap><Tag color={COLORS[run.phase]}>{PHASES[run.phase]}</Tag><Tag>{run.plan ? `${run.plan.id} · v${run.plan.version}` : '尚未生成计划'}</Tag><Typography.Text type="secondary">大脑：{['running', 'planning'].includes(run.phase) && run.handled_revision < run.revision ? '有事件待处理' : terminal(run.phase) ? '已结束' : '等待事件'}</Typography.Text><Typography.Text type="secondary">第 {run.activation} 次激活 · {connection === 'live' ? '实时连接' : connection}</Typography.Text><TimeText ts={run.updated_at} /></Space></div>
      ...
      <div className="brain-workspace"><aside className="brain-step-list">{(run.plan.plan.schema_version === 2 ? run.plan.plan.instances : run.plan.plan.steps || []).map((step) => { const group = run.groups.find((g) => g.id === step.id); const status = statusOf(group); return <button className={stepId === step.id ? 'selected' : ''} key={step.id} onClick={() => select(step.id)}><small>{step.action.kind}{step.foreach ? ' · 批量' : ''}</small><strong>{step.description || step.label}</strong><span>{STATES[status]}{group?.total > 1 ? ` · ${group.counts.succeeded || 0}/${group.total}` : ''}</span></button>; })}</aside>
