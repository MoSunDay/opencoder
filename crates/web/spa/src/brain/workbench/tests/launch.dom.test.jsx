...
it('uses v3 and run_id, preserving submission identity across an uncertain response', async () => {
  apiPost.mockRejectedValueOnce(new Error('connection lost')).mockImplementationOnce(async (_, body) => ({ schema_version: 3, run_id: body.id })); const created = vi.fn();
  ...
  fireEvent.click(screen.getByText('开始执行')); await waitFor(() => expect(created).toHaveBeenCalledWith(apiPost.mock.calls[0][1].id));
...
it('starts an explicit saved version with engineering input overrides and no mutable plan fields', async () => {
  ...
  apiPost.mockImplementation(async (_, body) => ({ schema_version: 3, run_id: body.id })); const created = vi.fn();
