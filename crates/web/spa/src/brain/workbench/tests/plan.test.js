...
describe('milestone plan versions', () => {
  ...
  it('explicit conversion preserves original version and requires success criteria', () => {
    ...
    expect(planLayers(converted.plan)).toEqual([['a'], ['b']]); expect(JSON.stringify(oldPlan)).toBe(before);
    expect(() => validatePlan(converted.plan, [capability])).toThrow('达成标准');
    const ready = { ...converted.plan, nodes: converted.plan.nodes.map((n) => ({ ...n, success_criteria: '通过检查' })) };
