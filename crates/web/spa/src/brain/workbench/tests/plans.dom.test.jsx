...
vi.mock('../canvas.jsx', () => ({ PlanCanvas: ({ plan, positions, onPositions, onViewport, onSelect }) => <div aria-label="测试画布"><span>{JSON.stringify(positions)}</span>{plan.instances.map((s) => <button key={s.id} onClick={() => onSelect?.(s.id)}>{s.description}</button>)}<button onClick={() => { onPositions({ fix: { x: 111, y: 222 } }); onViewport({ x: 30, y: 40, zoom: 0.8 }); }}>移动画布节点</button></div> }));
...
it('opens a full-width right drawer and restores the canvas, action text and viewport before any submission', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '修复—复测循环示例' }));
  ...
  expect(before.version.plan.routes).toHaveLength(2);
...
it('asks for name and summary only on submit, preserves routing, and clears the draft only after saving', async () => {
  mount(); open(); fireEvent.click(screen.getByRole('button', { name: '修复—复测循环示例' }));
  const cacheKey = key();
  ...
  expect(value.plan).toMatchObject({ title: '问题修复', objective: '复测发现问题就回退，全部通过再发布', schema_version: 2, entry: ['fix'] });
  expect(value.plan.routes[1].targets[0]).toMatchObject({ instance: 'fix', bindings: { feedback: 'verification' } });
  expect(value.plan.instances.every((s) => s.capability_id === 'cap-act')).toBe(true);
...
it('keeps failed submissions in browser storage and retries the same plan id after a page reload', async () => {
  ...
  let view = mount(); open(); fireEvent.click(screen.getByRole('button', { name: '修复—复测循环示例' }));
  fireEvent.click(screen.getByRole('button', { name: '提交计划' }));
...
it('retains invalid contract text across reloads and never submits the last valid plan silently', async () => {
  ...
  view.unmount(); view = mount(); open();
  fireEvent.click(screen.getByRole('tab', { name: '完整契约' }));
  expect(screen.getByLabelText('计划 JSON').value).toBe('{broken');
  expect(screen.getByRole('button', { name: '提交计划' }).disabled).toBe(true);
  expect(apiPost).not.toHaveBeenCalled();
  fireEvent.change(screen.getByLabelText('计划 JSON'), { target: { value: JSON.stringify(JSON.parse(localStorage.getItem(key())).version.plan) } });
