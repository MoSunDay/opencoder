...
export function ExecutionDetail({ id, summary, onClose, onNotice }) {
  ...
  useEffect(() => {
    ...
    return () => clearInterval(timer);
  }, [id, kind, index?.status, loadMessages, messageWindow]);
  const nextMessages = () => {
  ...
  return <Drawer open={!!id} title={id} onClose={onClose} size="large" styles={{ wrapper: { maxWidth: '100vw' } }}>
    ...
    <Space wrap style={{ margin: '12px 0' }}>
      ...
      {kind === 'project' && <><Button disabled={busy || unavailable || !projectRunnable} onClick={() => command('plan')}>生成计划</Button><Button disabled={busy || unavailable || !projectRunnable || !detail?.todo?.plan_md} onClick={() => command('execute')}>执行计划</Button></>}
    </Space>
    ...
    <WorkloadDetail id={id} detail={detail} kind={kind} />
