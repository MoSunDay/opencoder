// sha256.js — dependency-free SHA-256 + HMAC-SHA256 (FIPS 180-4 / RFC 2104).
// Browsers expose crypto.subtle only on secure contexts (https / localhost),
// so signing over plain-HTTP intranet origins needs a local fallback. Pure
// functions only: no classes, no module state, bytes in -> bytes out.
...
/// FIPS 180-4 SHA-256 over `data` -> 32 bytes.
...
/// RFC 2104 HMAC-SHA256: H((K^opad) || H((K^ipad) || m)) -> 32 bytes.
/// Keys longer than the 64-byte block are hashed first, per the RFC.
