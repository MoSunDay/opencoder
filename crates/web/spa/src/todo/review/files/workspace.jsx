...
export function ReviewFiles({id,snapshot,onTodo,onSession,onError}) {
  ...
  const alive=useRef(true);const pending=useRef(false);const callbacks=useRef({});callbacks.current={onError};
  const load=async(cursor)=>{
    if(pending.current)return;
    pending.current=true;setBusy(true);
    try{
      const page=await readReview(id,{section:'history',before_seq:cursor});
      const rows=await Promise.all(page.events.map(async event=>({...event,payload:await eventPayload(id,event)})));
      ...
      setError('');callbacks.current.onError?.('');
    }catch(e){if(alive.current){setError(e.message);callbacks.current.onError?.(e.message);}}
    finally{pending.current=false;if(alive.current)setBusy(false);}
  };
  useEffect(()=>{
    alive.current=true;
    readReview(id,{section:'files'}).then(result=>{
      if(!result.files)throw new Error('运行定义缺少文件');
      if(alive.current)setDefinition(result.files);
    }).catch(e=>{if(alive.current){setError(e.message);callbacks.current.onError?.(e.message);}});
  ...
  return <>
    {error&&<Alert type="error" title="读取运行文件失败" description={error}/>}
    <Space wrap style={{margin:'8px 0'}}>
      ...
      <Select aria-label="选择 TODO" placeholder="选择 TODO" showSearch optionFilterProp="label" style={{minWidth:200}} value={pathTodo(selected)||undefined}
        onChange={todo=>select(`process/todos/${todo}/status.json`)} options={(snapshot?.nodes||[]).filter(node=>!filter||node.status===filter).map(node=>({value:node.id,label:`${node.title} · ${node.status}`}))}/>
      <Button loading={busy} onClick={()=>load()}>刷新过程记录</Button>
    ...
    {!Object.keys(definition).length&&!error?<Spin/>:<FileWorkspace files={files} selected={selected} onSelect={select} readOnly/>}
  </>;
}
