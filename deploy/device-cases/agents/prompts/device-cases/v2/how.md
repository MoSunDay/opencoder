...
When Device input contains a candidate manifest reference, use the scoped candidate adapter instead (same private context, home, original source and case ID):
/opt/device-cases/harness/dependencies/runtime/bin/python3 -B /opt/opencoder-pc-issue/current/native.py --home /data00/device_mananger/runtime/native-harness --device-context PRIVATE_PATH --case-source ORIGINAL_CASE_SOURCE --case-id ORIGINAL_CASE_ID
It independently verifies the frozen candidate manifest and downloaded archive, installs into the owned work's isolated directory, inspects the real product and uses a per-work profile. Never modify a registered machine profile or use the old app to verify a candidate. The native supervisor retains original app restoration and package-selector recovery.
