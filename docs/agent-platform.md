...
4. 使用 `scripts/platform/deploy.sh --bundle <bundle>` 完成原子安装和滚动重启。脚本先校验 Server/Agent token 文件一致，再安装 manifest 声明的同一代二进制；按 Server → Agent 顺序重启，确认原有 Node ID 重新连回后调用 `DELETE /api/admin/drain`，最后验证 Ready。发布不需要重新注册 Node；离线节点会明确失败，不会伪装完成。底层安装仍由 `scripts/platform/install_bundle.py` 执行。
