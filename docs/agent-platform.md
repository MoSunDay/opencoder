...
6. 失败时立即重新冻结，不删除索引或执行数据。二进制回滚用 `scripts/platform/rollback.sh --bundle <上一代bundle> --dest-dir /usr/local/bin` 成套切换三者；需要回滚数据时先用 `scripts/platform/restore.sh --backup <备份> --output <新的隔离目录>` 验证并恢复到新目录，再让 Server/各 Node 的 `--data-dir` 明确指向对应恢复目录。不得把旧二进制直接指向未经配套验证的新协议数据，也不得覆盖原数据目录。
