...
## Context
OpenCoder 迭代一已建立 7-crate workspace 与基础 agent 运行时，但存储为无元数据的 JSONL、仅 web 层持久化、无恢复入口、模型/压缩配置僵化、web prompt 同步阻塞。迭代二补齐关键能力并建立测试金字塔（90% 覆盖目标 + glm5.2 e2e）。
...
- **性能（P5）**：release 启动 ~6ms；store 0.031ms/append、2.4ms/load1000、0.95ms/list200。见 [docs/perf.md](../../../../docs/perf.md)。
