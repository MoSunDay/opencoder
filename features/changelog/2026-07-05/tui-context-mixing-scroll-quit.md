...
## Change Summary
- **①系统提示词计入上下文（`app.rs`）**：新增 `sys_tokens_for(agent_name, workdir, skill)` —— 经 `opencode_session::prompt::build_system(...).text()` + `estimate` 估算系统提示词（agent.prompt + 环境块 + 当前 skill）token；`run_app` 初始化 `sys_tokens`，在 `SwitchAgent` / `SetSkill` 分支即时重算。`render_status` 显示 `context_used + sys_tokens`。`context_used`（转录流式累计、压缩时重置）与 `sys_tokens`（系统提示词，跨压缩常驻）分离，使百分比反映真实请求体积。
