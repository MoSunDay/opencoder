...
- CLI 与 MCP 每个注册项新增 `inject_to`：`parent`、`subagents`、`all`。旧配置缺省为 `parent`，保持原 MCP 仅父 Agent 可见的行为。
...
- `/cli` 与 `/mcp` 列表直接展示注入范围；编辑表单通过 Space/Enter 循环切换范围。配置保存后立即 reload，下一次模型调用生效。
