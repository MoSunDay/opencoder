...
- 22 个模块、155 用例（原 46 → 155），全部确定性（有界轮询/超时，无盲等）。
- 纯函数式：表驱动断言 + 无隐藏状态机，遵循仓库禁 class 规则；新文件均 <400 行、迭代文件 <800 行。

## Impact Surface

- 新增：`crates/control/tests/e2e/`（22 模块 + support 基建，共 ~9.6k 行，全部测试代码）。
- 不改任何生产代码；仅测试基建与用例。
...
| admin drain：冻结-重开循环、活跃执行聚合、offline_nodes、重开拒绝保持冻结、未确认 freeze | admin_drain.rs、fleet_admin_extra.rs | 6 |
| fleet：节点目录派生（idle/busy/lost）、维护 RPC 透传与准入门、WS 错误 Bearer | fleet_maintenance.rs、fleet_admin_extra.rs | 10 |
...
| dag：defs 校验表/裸 spec/bare delete、派发自增 id/幂等/前缀冲突/固定/428 重试、列表降级、after 游标、workflow 404 | dag_runs.rs、teams_dag_defs.rs、dag_dispatch_extra.rs | 16 |
