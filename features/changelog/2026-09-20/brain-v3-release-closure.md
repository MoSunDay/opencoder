...
首次接收的资源准备按执行 ID 串行、按节点限制四路复制，冷文件读取不再占用全局接收锁。WASI-only DAG 绕过 Agent 资源复制槽位；暂停接收与最终入队再次核验状态。准备前由执行节点原子持久化原请求，重启重试沿用原定义、输入和创建时间，拒绝同 ID 改参，成功入队后由执行日志接管该记录。

同步资源准备在多线程 Runtime 中移交异步工作线程，保留执行的任务局部资源根与线程局部配置隔离；单工作线程回归核验冷准备期间其他任务仍能运行。WASI-only DAG 不检查无关的 Agent 挂载。

部署器恢复同一激活轮次时，核对节点已持久化的探针身份、原请求和定义，避免重复创建已接收的探针；仍查询真实 Runtime 的成功终态与就绪状态。已有记录与本轮探针不一致时直接失败。

大脑创建重试通过原节点与持久接收回执确认成功，不能把仅冻结的派发记录当作节点已接收。Project 的同一执行根允许在明确拒绝后提交新 run_id；未确认准备和已接收请求仍保持参数不可变，旧尝试的数据不删除。
