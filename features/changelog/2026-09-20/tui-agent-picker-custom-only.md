...
9 项交互再次通过，7 次进程日志均无 ERROR 或 panic。正在执行新的版本绑定
900 秒观察，起点、中段、终点均执行相同交互；如安装二进制或发布版本改变，
观察即失效。
并发平台发布使随后观察窗口在约 195 秒时失效；该作业在回滚基线就绪及任务
...
自定义项及 `/act`、`/plan` 持久化；这不替代正式发布窗口验收。仍需通过连续性门槛和 900 秒观察，
