...
- **TUI**：3-region 布局（body / composer / 底部合并 status：model + agent + dir + ctx%）+ ratatui Scrollbar + 自动跟随 + 运行中 braille spinner 动画 + 可见 bar 光标 + 实时上下文百分比 + subagent 渲染 + steer/followup（Enter=steer / Tab=follow-up）+ 双击 Esc 硬中止 + thinking 默认折叠 + say: 助手标识 + Alt+Tab 模式切换（Ctrl+T fallback）+ skill 模糊匹配（`$` 选择器）+ 每 session 独立状态（`/task` 切换保存/恢复 chat+scroll+history）。快捷键：Alt+Tab 切 plan/act、Ctrl+D 退出、Esc 关 help。详见 [迭代三](changelog/2026-07-05/iteration3-tui-overhaul.md) / [迭代四](changelog/2026-07-05/iteration4-tui-ux-hardabort.md) / [subagent 重构](changelog/2026-07-05/subagent-and-tool-refactor.md)。
...
- **模型配置（TUI `/model`）**：空 composer 输入 `/` 弹出 slash 命令选择器（`/task` 切会话、`/model` 配模型）。`/model` 模态可编辑 model / base_url / api_key（掩码 `sk-****1234`，编辑不回显）/ reasoning_effort（off↔low↔medium↔high 四档）/ compaction.context_threshold；保存经 `Config::save` 持久化到 opencode.json（项目优先）并热重载——`UiCmd::ReloadConfig` 在 turn 边界替换 `session.config`/`session.model` 并重建 `ChatClient`。状态栏 model 旁显示 `·high` 思考深度徽标。详见 [agents/core](../agents/core/index.md)、[agents/llm](../agents/llm/index.md) / [model-command changelog](changelog/2026-07-05/model-command-and-reasoning-effort.md)。
- **测试规则与覆盖**：[rules/](../rules/) 目录建立强制测试规则（每功能必有测试 + 迭代回归 gate + 测试分层）；workspace 测试 198 个，覆盖 LLM 流式原语 / CLI / 工具 / subagent / bash_guard / TUI / Web / prompt / store CRUD。详见 [迭代四 changelog](changelog/2026-07-05/iteration4-test-coverage-rules.md)。
