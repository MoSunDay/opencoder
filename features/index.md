...
- **glm5.2 端到端**：`scripts/e2e-glm.sh` 用真 glm5.2 覆盖 9 项跨切面契约：E7 reasoning_effort 显示、E1/E6 write+bash 写游戏并编译、E2 `--continue` 恢复、E5 `--fork` 不污染原 session、E3/E3b 压缩自动触发+压缩后恢复、E4 subagent (task tool) 派遣、E8 bundle 导出/导入 roundtrip。
...
- **测试规则与覆盖**：[rules/](../rules/) 目录建立强制测试规则（每功能必有测试 + 迭代回归 gate + 测试分层）；workspace 测试 288 个，覆盖 LLM 流式原语 / CLI / 工具 / subagent / bash_guard / TUI / Web / prompt / store CRUD / bundle 导出导入 / fork / interleaved thinking。详见 [迭代四 changelog](changelog/2026-07-05/iteration4-test-coverage-rules.md)。
