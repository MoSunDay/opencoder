...
def exercise(args, settings, root):
    ...
    try:
        ...
        with (root / 'deployment.log').open('wb') as log:
            subprocess.run([sys.executable, str(Path(__file__).resolve().parents[2] / 'platform/rolling_cli.py'),
                '--config', str(args.config), '--bundle', str(args.bundle), '--wait-seconds', '180'],
