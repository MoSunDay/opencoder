...
def deploy(settings, bundle, operations, seconds=90):
    ...
    if journal.data["current"] == identifier and journal.data["phase"] in ("complete", "rolled_back"):
        ...
        return journal.data
    if identifier not in journal.data["releases"]:
...
def rollback(settings, operations, seconds=90):
    ...
    if journal.data["phase"] != "rolling_back":
        ...
        used = [int(r[k]) for r in journal.data["releases"].values() for k in ("host_port", "server_port", "runtime_port")]
        used.extend(h["port"] for r in journal.data["releases"].values() for key in ("previous_hosts", "previous_servers") for h in r.get(key, []))
        old.setdefault("previous_hosts", []).append({"unit":old["host_unit"],"port":old["host_port"]})
        old.setdefault("previous_servers", []).append({"unit":old["server_unit"],"port":old["server_port"]})
        old["host_port"] = max(used) + 1
        old["server_port"] = max(used) + 2
        if old["server_port"] >= 65536:
